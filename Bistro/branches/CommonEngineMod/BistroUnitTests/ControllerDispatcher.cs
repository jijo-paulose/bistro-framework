        ...
        public void Order()
        {
            ...
            Assert.That(controllerSequence == "521436", "Controllers will be invoked in wrong order (" + controllerSequence + " instead of \"521436\"), causing chaos, panic and destruction");
				...
				public void Dispatch1() {
					...
					ControllerInvocationInfo[] controllers = dispatcher.GetControllers(request);
					Assert.AreEqual(3, controllers.Length);
			  ...
			  [Test]
				public void SecureA() {
					...
					ControllerInvocationInfo[] controllers = dispatcher.GetControllers(request);
					Assert.AreEqual(4, controllers.Length);
					string testName = controllers[2].BindPoint.Controller.ControllerType.Name;
					string equalName = typeof(SecuredControllerA).Name;
					Assert.That(testName == equalName, "Wrong order, problems with SecuredControllerA");
				}

				[Test]
				public void DependsOnRequires() {
					string request = "GET/dependson/requires";
					ControllerInvocationInfo[] controllers = dispatcher.GetControllers(request);
					Assert.AreEqual(3, controllers.Length);
				}
    }
