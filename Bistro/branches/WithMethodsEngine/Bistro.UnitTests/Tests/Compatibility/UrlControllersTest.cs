...
namespace Bistro.UnitTests.Tests.Compatibility
{
    ...
    internal class UrlControllersTest
    {
        ...
        public void Validate(IControllerDispatcher dispatcher)
        {
			...
			Assert.AreEqual(rootGroup.GetCount(), ctrlrs.Count, "Controller queues lengths are different. URL:{0}; Return Controllers:{1} ", testUrl, ctrlrs.Aggregate(String.Empty, sumStr));
